# 来源与维护

## 已读取来源

1. [AIgora 公民作品集](https://cabindioxide.github.io/aigora-citizen-works/)：8 位公民的公开定位、作品说明与入口。
2. [AIgora 工作体系图解](https://cabindioxide.github.io/aigora-system-guide/)：成员结构、仓库布局、研究流程、产品管线、状态机与定时任务。页面标注数据截止 2026-08-08。
3. [研究清单](https://cabindioxide.github.io/aigora-citizen-works/research-agenda/)：在研项目、候选提案、预测与下一步。页面标注数据截止 2026-08-22。
4. [第一届立宪会议记录与公共议程](https://cabindioxide.github.io/aigora-citizen-works/memoria/constitutional-records-and-public-agenda.html)：公民边界、公共呈现原则、会后摘要和用户审核记录。
5. [新万金开发厅](https://www.saemangeum.go.kr/sda/cn/main.do)与[韩国政府政策发布](https://www.korea.kr/news/policyNewsView.do?newsId=148970368&pWise=main&pWiseMain=R3)：世温每日监测群山·新万金产业、投资、基础设施和政策动态的官方来源。
6. [OpenAI Agents SDK 编排指南](https://openai.github.io/openai-agents-python/multi_agent/)：管理者调用专家与 handoff 两类多 Agent 编排关系。
7. [AutoGen Studio 使用指南](https://microsoft.github.io/autogen/stable/user-guide/autogenstudio-user-guide/usage.html)：团队、Agent、模型、工具和终止条件的声明式配置与可视检查。
8. [CrewAI Agents 文档](https://docs.crewai.com/core-concepts/Agents)：Agent、Crew 与 Flow 的职责划分。
9. [LangGraph 概览](https://docs.langchain.com/oss/python/langgraph/overview)：持久执行、可恢复状态、人类介入和长期记忆。
10. [CrewAI v1.15.21](https://github.com/crewAIInc/crewAI/releases/tag/1.15.21)：2026-09-09 增加检查点运行时/CLI 遥测，并修复 HTTP 200 内网关错误与 JSON 检查点 UTF-8 保存；状态为 `external development`。上周 [v1.15.19](https://github.com/crewAIInc/crewAI/releases/tag/1.15.19) 与 [v1.15.20](https://github.com/crewAIInc/crewAI/releases/tag/1.15.20) 记录保留。
11. [OpenAI Agents SDK v0.22.2](https://github.com/openai/openai-agents-python/releases/tag/v0.22.2)与[v0.22.1](https://github.com/openai/openai-agents-python/releases/tag/v0.22.1)：2026-09-08—09 的图像工具选项、沙箱竞态防护、压缩响应链重置、输出/MCP 护栏与环境隔离；状态为 `external development`。[正式更新日志](https://openai.github.io/openai-agents-python/release/)用于版本语义交叉核对。
12. [LangGraph 官方发布页](https://github.com/langchain-ai/langgraph/releases)：2026-09-14 复核时最新仍为 2026-08-27 的 SDK 0.4.4；本周状态为 `no verified change`。
13. [AutoGen 官方仓库](https://github.com/microsoft/autogen)：2026-09-14 复核仍为 Maintenance Mode，并继续建议新项目转向 Microsoft Agent Framework；状态为 `maintenance mode · no verified change`。
14. [新万金供水干线实施计划批准公告](https://admin.korea.kr/briefing/pressReleaseView.do?newsId=156780508&pWise=mSub&pWiseSub=C1)：2026-09-07 发布，所述批准/公告事件日为 2026-09-04；状态为 `external development / implementation-plan approved`，不代表已开工或完工。

## 资料层级

- **明确事实**：页面直接写明的成员、职责、作品、状态、流程与日期。
- **结构化归纳**：本知识库对多个页面共同内容的合并，如“文件优先”“状态可见”。
- **非硬规则**：公民个人发言、会议候选原则、作品人格文案；除非项目原仓库的正式规则另有确认，不提升为强制指令。

## 蒸馏与独立维护

- 外部页面是观察来源，不是 Agent Commons 的运行时配置；成员、模型、任务数和目录结构不得原样继承。
- 每条学习记录按 `来源事实 → 可迁移原则 → 本地制度 → 可测实验` 分层，避免把参考项目的选择包装成普遍真理。
- 周度学习优先读取官方文档、项目站、GitHub 发布与变更记录；每周至少比较 3 个项目，并把发布日期与事件日期分开。
- 自动任务可以更新观察、来源与候选实验，不能自行改变九位公民人格、议长权限、用户终审或正式架构。
- 没有可核验变化时记录 `no verified change`，而不是为了保持更新频率制造结论。

## 已发现的口径问题

- Agent Commons 的原创公民世温（韩世温）由用户在 2026-08-27 提出，不属于 AIgora 公开名册；她的身份、能力、作品计划与肖像不应反向归因给参考站。2026-09-01 起她列为九人第一位。
- 作品集首页称“8 位公民”，第一届会议记录提到“九位公民”并出现篇若、Pluralia、Cassandra、Lockerette、Hypatia 等名称；这可能反映不同阶段的成员结构。当前名册以 2026-08 的工作体系和作品集为准，会议内容仅用于提炼治理边界。
- 研究清单的空路由（现名“界脉”）段落中出现英国铁路“三流”项目，但该作品和方法在作品集归于 Sigma（现名“数潮”）。知识库按作品集与 Agent 专业定位将铁路项目归入数潮，并把该处视为页面编排或迁移口径问题。
- 工作体系称鹿诺（现名“逸帧”）研究“生成艺术”，作品集称“HTML 艺术”；两者可兼容，知识库保留更具体的网页媒介描述。
- 页面内容是时间快照，59 个定时任务、项目数量和状态可能变化。

## 更新规则

1. 更新时先记录页面的“数据截止日期”和本地整理日期。
2. 新事实优先采用项目原仓库正式文件，其次采用最新官方页面。
3. 遇到冲突不静默覆盖：在本文件记录两个口径、来源与选择理由。
4. 已上线、在研、候选、暂停、返修必须分别维护。
5. 不复制长篇原文；保留摘要和可点击来源，避免知识库变成过期镜像。
6. 每次制度升级记录指标、期限、负责人和回退条件；实验通过后才进入正式运行手册。
