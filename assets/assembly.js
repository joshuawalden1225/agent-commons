const assemblyUI = {
  zh: {
    locale:'zh-CN', title:'Agent Commons · 九人议事厅', data:'assets/agents.json?v=20260922-energy',
    ...
    briefTitle:'最近研究简报', briefIntro:'汇总议题、跨域借鉴、九人研究观察与下一步行动。打印时自动整理为单页 A4。', printBrief:'打印 / 保存 A4 PDF', dailyBrief:'RESEARCH RECORD · AS OF', crossLearning:'跨域借鉴', decisions:'会议决定', researchWatch:'九人研究观察', evidenceNote:'证据说明：研究摘要来自已登记记录，不随访问日期自动变成新成果；会议交流与决议为模拟。实际交付及独立复核请查看研究工作台。', provenanceIndex:'03 / SOURCE REGISTER', provenanceTitle:'来源与修订记录', provenanceIntro:'查看议事厅公开主张的证据状态、复核者、来源文件与下一复核日期。私有日报不会在此公开。', provenanceStatus:'状态', provenanceReviewer:'复核者', provenanceReviewDate:'下一复核', provenanceEvidence:'来源', provenanceValid:'来源 schema 检查通过 · 0 项缺失', provenanceIssues:n=>`来源 schema 检查发现 ${n} 项问题`, withdrawalTitle:'撤回演练 · 非公开测试记录', withdrawalNote:'该条目用于验证撤回与替代路径，不代表网站曾经公开过这一错误主张。', footer:'对话留下记录，判断保留异议，行动等待核验。', home:'返回公地首页 ↗',
    agendas:[
  ...
  en: {
    locale:'en', title:'Agent Commons · Nine-Citizen Assembly', data:'assets/agents.en.json?v=20260922-energy',
    ...
    briefTitle:'Latest research brief', briefIntro:'Agenda, cross-domain learning, nine research watches, and next actions—formatted as one A4 page when printed.', printBrief:'Print / save A4 PDF', dailyBrief:'RESEARCH RECORD · AS OF', crossLearning:'Cross-domain learning', decisions:'Decisions', researchWatch:'Nine research watches', evidenceNote:'Evidence note: summaries reflect dated archive records, not new work on the visit date. Exchanges and decisions are simulated. See the research workbench for actual deliveries and independent review.', provenanceIndex:'03 / SOURCE REGISTER', provenanceTitle:'Sources and revision record', provenanceIntro:'Inspect the evidence state, reviewer, source files, and next review date for public assembly claims. Private daily reports are not exposed here.', provenanceStatus:'Status', provenanceReviewer:'Reviewer', provenanceReviewDate:'Next review', provenanceEvidence:'Sources', provenanceValid:'Provenance schema passed · 0 missing fields', provenanceIssues:n=>`Provenance schema found ${n} issue(s)`, withdrawalTitle:'Withdrawal rehearsal · non-public test', withdrawalNote:'This entry tests withdrawal and supersession paths; it does not mean the incorrect claim was ever published.', footer:'Dialogue leaves a record; judgment keeps dissent; action awaits verification.', home:'Return to the Commons ↗',
    agendas:[
  ...
  ko: {
    locale:'ko', title:'Agent Commons · 아홉 시민 의회', data:'assets/agents.ko.json?v=20260922-energy',
    ...
    briefTitle:'최근 연구 브리프', briefIntro:'의제, 분야 간 학습, 아홉 연구 관찰, 다음 행동을 인쇄 시 A4 한 장으로 정리합니다.', printBrief:'인쇄 / A4 PDF 저장', dailyBrief:'RESEARCH RECORD · AS OF', crossLearning:'분야 간 학습', decisions:'회의 결정', researchWatch:'아홉 연구 관찰', evidenceNote:'근거 안내: 요약은 날짜가 명시된 기록이며 방문일의 새 성과가 아닙니다. 교류와 결의는 시뮬레이션입니다. 실제 산출물과 독립 검토는 연구 작업대에서 확인하세요.', provenanceIndex:'03 / SOURCE REGISTER', provenanceTitle:'출처와 수정 기록', provenanceIntro:'의회 공개 주장의 근거 상태·검토자·출처 파일·다음 검토일을 확인합니다. 비공개 일일 보고서는 여기에서 공개하지 않습니다.', provenanceStatus:'상태', provenanceReviewer:'검토자', provenanceReviewDate:'다음 검토', provenanceEvidence:'출처', provenanceValid:'출처 schema 검사 통과 · 누락 0건', provenanceIssues:n=>`출처 schema에서 문제 ${n}건 발견`, withdrawalTitle:'철회 연습 · 비공개 테스트', withdrawalNote:'이 항목은 철회와 대체 경로를 검증하는 테스트이며 잘못된 주장이 실제 공개됐다는 뜻이 아닙니다.', footer:'대화는 기록을 남기고, 판단은 이견을 보존하며, 행동은 검증을 기다립니다.', home:'공유지 홈으로 돌아가기 ↗',
    agendas:[
...
let researchUpdatedAt = "";
let provenanceManifest = {records:{}};
let selectedAgent = 0;
let dialogueTimer;
...
function renderProfile(index) {
  ...
  document.querySelector('#profile-capabilities').innerHTML = agent.capabilities.map(capability => `<div><span>${escapeHTML(capability.name)}</span></div>`).join('');
  ...
  if(agent.id==='saeon')document.querySelector('#profile-frontiers').insertAdjacentHTML('afterbegin', `<a class="work-panel-link" href="energy.html?lang=${activeLang}">${({zh:'全球能源观察站 · 周度研究 →',en:'Global Energy Observatory · Weekly →',ko:'글로벌 에너지 관측소 · 주간 연구 →'})[activeLang]}</a>`);
  document.querySelector('#profile-frontiers').insertAdjacentHTML('afterbegin', `<a class="work-panel-link" href="research.html?citizen=${agent.id}&lang=${activeLang}">${({zh:'查看当前任务、交付标准与成果 →',en:'Current tasks, acceptance criteria & results →',ko:'현재 과제·완료 기준·성과 보기 →'})[activeLang]}</a>`);
...
function provenanceErrors(manifest) {
  ...
  if (!manifest || !manifest.records || typeof manifest.records !== 'object') return ['records'];
  Object.entries(manifest.records).forEach(([id, record]) => {
    required.forEach(field => {
      if (record[field] === undefined || record[field] === null || record[field] === '') errors.push(`${id}.${field}`);
    });
    ['zh','en','ko'].forEach(language => {
...
function provenanceHref(source) {
  if (/^https?:\/\//.test(source)) return source;
  if (/^(assets|knowledge-base)\//.test(source)) return source;
  return '#provenance';
}

function renderProvenance(copy) {
  const errors = provenanceErrors(provenanceManifest);
  const health = document.querySelector('#provenance-health');
  health.textContent = errors.length ? copy.provenanceIssues(errors.length) : copy.provenanceValid;
  health.dataset.state = errors.length ? 'error' : 'valid';
  health.dataset.errors = errors.join(',');
  document.querySelector('#provenance-grid').innerHTML = Object.entries(provenanceManifest.records).map(([id, record]) => {
    const claim = record.claim?.[activeLang] || record.claim?.en || '';
    const sources = record.evidence.map(source => `<li><a href="${escapeHTML(provenanceHref(source))}">${escapeHTML(source)}</a></li>`).join('');
    return `<article class="provenance-card"><header><span>${escapeHTML(id)}</span><b>${escapeHTML(record.status)}</b></header><p>${escapeHTML(claim)}</p><dl><div><dt>${escapeHTML(copy.provenanceReviewer)}</dt><dd>${escapeHTML(record.reviewer)}</dd></div><div><dt>${escapeHTML(copy.provenanceReviewDate)}</dt><dd>${escapeHTML(record.nextReview)}</dd></div></dl><details><summary>${escapeHTML(copy.provenanceEvidence)} · ${record.evidence.length}</summary><ul>${sources}</ul></details></article>`;
  }).join('');
  const rehearsal = provenanceManifest.withdrawalRehearsal;
  document.querySelector('#withdrawal-rehearsal').innerHTML = rehearsal ? `<div><span>${escapeHTML(copy.withdrawalTitle)}</span><b>${escapeHTML(rehearsal.status)}</b></div><p>${escapeHTML(copy.withdrawalNote)}</p><small>${escapeHTML(rehearsal.recordId)} → ${escapeHTML(rehearsal.supersededBy)}</small>` : '';
}
...
function renderBrief(copy, agenda, exchanges, count, seed) {
  const date = researchUpdatedAt || '—';
  document.querySelector('#brief-date').textContent = date;
  document.querySelector('#brief-agenda').textContent = agenda.title;
  const notes = {zh:'研究记录截至 '+date+'；下列交流与决议为页面会议模拟。',en:'Research records as of '+date+'; exchanges and decisions below are the page meeting simulation.',ko:date+' 기준 연구 기록. 아래 교류·결의는 페이지 회의 시뮬레이션입니다.'};
  document.querySelector('#brief-thesis').textContent = notes[activeLang];
  ...
  document.querySelector('#brief-progress').innerHTML = agents.map(agent => {
    const entry = researchFrontiers[agent.id]?.[activeLang]?.[0];
    const labels={zh:'暂无研究记录',en:'No research record',ko:'연구 기록 없음'};
    return `<div class="progress-line"><strong>${escapeHTML(agent.name)}</strong><span>${escapeHTML(entry?.eventDate || '—')}</span><p>${escapeHTML(entry?.summary || labels[activeLang])}</p><a href="research.html?citizen=${agent.id}&lang=${activeLang}">${escapeHTML(entry?.next || labels[activeLang])} →</a></div>`;
  }).join('');
...
async function setLanguage(language) {
  ...
  const [response, frontiersResponse, provenanceResponse] = await Promise.all([
    fetch(copy.data),
    fetch('assets/research-frontiers.json?v=20260922-energy'),
    fetch('assets/provenance.json?v=20260914-1')
  ]);
  ...
  researchFrontiers = frontiersData.citizens || {};
  researchUpdatedAt = frontiersData.updatedAt || '';
  provenanceManifest = await provenanceResponse.json();
